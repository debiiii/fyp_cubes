...
public class OpenGLRenderer_SPType3_Quest implements GLSurfaceView.Renderer {
    ...
    QuestionBank_SPType3_Quest questionBank_spType3_quest = new QuestionBank_SPType3_Quest();
    private static boolean canDraw = false;

    public OpenGLRenderer_SPType3_Quest(){
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if(canDraw){
            ...
            for(int i = 0; i < MAXGRIDSNUM; i++){
                for(int j = 0; j < MAXHEIGHTNUM; j++){
                    if(questionBank_spType3_quest.getIsCubePresent(i, j)){
