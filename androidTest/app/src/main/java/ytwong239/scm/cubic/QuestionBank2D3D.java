...
public class QuestionBank2D3D {

    private static final int MAXQUESTBANK2D3DSUM = 7;
    private Question2D3D question2D3DS[] = new Question2D3D[MAXQUESTBANK2D3DSUM];

    private static int currQuestBank2D3DNum;
    
    public QuestionBank2D3D(){
        for(int i = 0; i < MAXQUESTBANK2D3DSUM; i++){
            question2D3DS[i] = new Question2D3D();
        }
